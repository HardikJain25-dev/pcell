    ...
    <>
      ...
      >
        ...
        <div className="w-[90vw] max-w-xs md:max-w-4xl mx-auto">
          <div className="bg-white/10 backdrop-blur-lg border border-white/15 rounded-full px-4 py-3 md:px-6 md:py-2 dark:text-black dark:hover:text-black">
            <div className="flex items-center justify-between">
              ...
              >
                <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-between gap-1">
                  <Image
                    src="/pcell.webp"
                    alt="Cliste"
                    width={40}
                ...
                {/* <div className="text-white/80 bold text-xl" >The Placement Cell</div> */}
              {/* Desktop Navigation */}
                ...
                {navigation.map((item) =>
                  item.href.startsWith("/") ? (
                    <Link
                      key={item.name}
                      href={item.href}
                      className="text-black hover:text-black hover:scale-115 transition-all duration-200 font-medium cursor-pointer dark:text-black dark:hover:text-white"
                    >
                      {item.name}
                    </Link>
                  ) : (
                    <button
                      ...
                      className="text-black hover:text-slate hover:scale-105 transition-all duration-200 font-medium cursor-pointer"
                    >
                      {item.name}
                    </button>
                  ),
              ...
              <button
                ...
                className="md:hidden text-black hover:scale-110 transition-transform duration-200 cursor-pointer dark:text-white "
              >
        ...
        <div className="md:hidden relative">
          {/* Backdrop overlay */}
          <div
            className={`fixed inset-0  backdrop-blur-lg transition-all duration-300 ${
          ...
          >
            ...
            <div className="bg-white/0 backdrop-blur-md border border-white/20 rounded-2xl p-4 shadow-2xl ">
                ...
                {navigation.map((item, index) =>
                  item.href.startsWith("/") ? (
                    <Link
                      key={item.name}
                      href={item.href}
                      className={`text-black hover:text-white hover:bg-white/10 rounded-lg px-3 py-3 text-left transition-all duration-300 font-medium cursor-pointer transform hover:scale-[1.02] hover:translate-x-1 ${
